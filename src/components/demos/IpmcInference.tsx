 ...
 */
function predict({ voltage, env, time }: { voltage: number; env: "water" | "air" | "acetone"; time: number }) {
...
export default function IpmcInference() {
  ...
  const W = 400, H = 160, padL = 28, padB = 22;
  const xScale = (t: number) => padL + (t / 10) * (W - padL - 8);
  const yScale = (a: number) => H - padB - ((a + 5) / 45) * (H - padB - 8);

  const path = curve.map((p, i) => `${i === 0 ? "M" : "L"}${xScale(p.t).toFixed(1)},${yScale(p.a).toFixed(1)}`).join(" ");
  const tMark = xScale(time);

  return (
    <div className="pixel-border pixel-bevel bg-obsidian/80 p-4">
      <div className="flex items-center justify-between mb-2">
        <div className="font-display text-[10px] text-diamond text-shadow-pixel">LIVE DEMO · IPMC INFERENCE</div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <div className="space-y-3" style={{ fontFamily: "var(--font-hud)", fontSize: 16 }}>
          <Slider label="Voltage" unit="V" min={2.1} max={4.9} step={0.05} value={voltage} onChange={setVoltage} />
          <Slider label="Time" unit="s" min={0} max={10} step={0.1} value={time} onChange={setTime} />
          <div>
          ...
          <div className="pixel-border bg-stone/40 p-3">
            ...
            <div className="font-display text-xl text-grass text-shadow-pixel">{out.toFixed(2)}°</div>
          </div>
        </div>

        <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-auto bg-sky-night pixel-border" aria-label="Phase-space angle vs time">
          {/* gridlines */}
          {[0, 10, 20, 30].map((a) => (
            <line key={a} x1={padL} y1={yScale(a)} x2={W - 8} y2={yScale(a)} stroke="rgba(255,255,255,0.08)" />
          ))}
          {/* axes */}
          ...
          {/* axis labels */}
          <text x={padL} y={H - 4} fill="#9aa0a6" fontSize="10" fontFamily="var(--font-hud)">0s</text>
          <text x={W - 18} y={H - 4} fill="#9aa0a6" fontSize="10" fontFamily="var(--font-hud)">10s</text>
          <text x={4} y={14} fill="#9aa0a6" fontSize="10" fontFamily="var(--font-hud)">30°</text>
          <text x={4} y={H - padB} fill="#9aa0a6" fontSize="10" fontFamily="var(--font-hud)">0°</text>
        </svg>
      </div>
    </div>
  );
  ...
  return (
    <label className="block">
      <div className="flex justify-between text-white/70">
        <span>{label}</span>
        <span className="text-diamond">{value.toFixed(2)} {unit}</span>
      </div>
      <input
        type="range"
